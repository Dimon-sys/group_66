...
def send_game(message):
    bot.send_message(message.chat.id, 'Я бы порекомендовал Astroneer.А вообще какой вы жанр предпочитаете?')
def send_sandbox(message):
    bot.send_message(message.chat.id, 'Minecraft - ЭтО мОя ЖиЗнЬ !!!!')
def send_horror(message):
    bot.send_message(message.chat.id, 'Outlast - кирпичный завод обеспечен')
def send_race(message):
    bot.send_message(message.chat.id, 'Need for Speed: Most Wanted')
def send_shooter(message):
    bot.send_message(message.chat.id, 'Doom: Eternal')
...
def answer(message):
    if message.text.strip() == 'Факт':
        ...
        send_game(message)
    if message.text.strip() == 'Песочница':
        send_sandbox(message)
    elif message.text.strip() == 'Хоррор':
        send_horror(message)
    elif message.text.strip() == 'Гонки':
        send_race(message)
    elif message.text.strip() == 'Шутер':
        send_shooter(message)
